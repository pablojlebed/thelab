<script lang="ts">
            ...
            .subscribe(async (status) => {
                if (status === "SUBSCRIBED") {
                    await realtimeChannel.track({
                        ...
                        // Generate a color based on name (simple hash)
                        color: stringToColor($currentUser?.name || "Guest"),
                    });
                }
            });
    }
    ...
    function handleTaskChange(payload: any) {
        ...
        if (eventType === "INSERT") {
            // Check deduplication (if we added it optimistically already)
            // But strict ID check usually suffices if ID was generated by DB.
            if (!allTasks.find((t) => t.id === newRecord.id)) {
                allTasks = [transformTask(newRecord), ...allTasks];
            }
        } else if (eventType === "UPDATE") {
            allTasks = allTasks.map((t) =>
                t.id === newRecord.id ? transformTask(newRecord) : t,
            );
        } else if (eventType === "DELETE") {
            allTasks = allTasks.filter((t) => t.id !== oldRecord.id);
        }
    }
    ...
    function transformTask(dbTask: any) {
        return {
            ...dbTask,
            owner: dbTask.superhero,
            description: dbTask.mission,
        };
    }
    ...
    async function handleCreate(e: CustomEvent) {
        ...
        // Add to main list directly (letting reactivity handle columns)
        // We'll trust Realtime to confirm, but for instant feedback we add it.
        // HOWEVER, handleCreate waits for DB response, so we have the real ID.
        // We can just add it to allTasks.

        allTasks = [mappedTask, ...allTasks];
        showModal = false;
    }
    ...
    async function handleUpdate(e: CustomEvent) {
        ...
        // Optimistic update via allTasks
        allTasks = allTasks.map((t) =>
